...
ウィンドウ右上に「_(最小化)」「✕(閉じる)」の2つのボタンがあります。「最小化」はタスクバーへの通常の最小化、「✕」はウィンドウを閉じてトレイに格納します(どちらもプロセスは終了しません)。トレイアイコンをクリックすると再表示、右クリックで「開く」「終了」を選べます。アプリを完全に終了するには、トレイメニューの「終了」を使います。
