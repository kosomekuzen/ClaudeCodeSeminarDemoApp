...
ウィンドウ右上に「⚙(設定)」「_(最小化)」「✕(閉じる)」の3つのボタンがあります。「最小化」はタスクバーへの通常の最小化、「✕」はウィンドウを閉じてトレイに格納します(どちらもプロセスは終了しません)。トレイアイコンをクリックすると再表示、右クリックで「開く」「終了」を選べます。アプリを完全に終了するには、トレイメニューの「終了」を使います。
...
- **起動・常駐**: 常に最前面に表示、PC起動時に自動で立ち上げる、ホットキー(`Ctrl(⌘) + Shift + Space`)での呼び出し、ウィンドウの位置・サイズの記憶
- **実行**: 起動前の確認をスキップする(「開始」を押すと即座に実行)、実行完了時のOS通知
- **データ**: 業務モードのバックアップ(JSON形式での書き出し/読み込み。他のPCへの引き継ぎに使える)

「PC起動時に自動で立ち上げる」はElectronの`app.setLoginItemSettings`を使っており、Windowsのスタートアップフォルダを手動で操作する必要はありません。
