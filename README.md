...
**「Electron本体(electron.exe)が見つかりません」と出る / アプリが起動しない**

```bash
npm run setup
```

`electron` パッケージは、インストール後に別途Electron本体(約100MB)をダウンロードします。社内プロキシやウイルス対策ソフトがこの通信を遮断すると、**`npm install` 自体は成功したように見えるのに本体だけが無い**状態になります。さらに一度この状態になると npm は「up to date」と判断するため、`npm install` をやり直しても直りません。上のコマンドは本体の有無を確認し、無ければ取得し直します。

**フォルダを削除・移動できない(「別のプログラムがこのフォルダーまたはファイルを開いています」)**

アプリが常駐したままです。ウィンドウの「✕」はトレイに格納するだけで終了しません。常駐アイコンを右クリック →「終了」で完全に終了してから操作してください(それでも残る場合は、タスクマネージャーで `electron.exe` を終了)。

**置き場所について**

OneDrive配下は避けることをおすすめします。`node_modules` は数万ファイル規模になるため同期が重くなり、ファイルが掴まれて削除・更新に失敗しやすくなります。
